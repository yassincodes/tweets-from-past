export const arrayOfCharacters = [
  {
    character: "أدولف هتلر",
    character_pic:
      "https://cdn.britannica.com/57/146057-050-697A470D/Adolf-Hitler.jpg",
    username: "@realAdolfHitler",
    bio: (
      <div>
        رسام, سياسي, قائد عسكري وكاتب
      </div>
    ),
    cover_pic: "https://learndojo.org/wp-content/uploads/2019/08/GCSE-History-Life-in-Nazi-Germany-1-768x518.jpg",
    birthday: "1889 "  + " 30" + " أفريل",
    location: "ألمانيا",
    website: "https://en.wikipedia.org/wiki/Adolf_Hitler",
    character_link: "realAdolfHitler",
  },
  {
    character: "وينستون تشرشل",
    character_pic:
      "https://cdn.britannica.com/25/171125-050-94459F61/Winston-Churchill.jpg",
    username: "@Churchill_UK",
    bio: (
      <div>
        مؤرخ, كاتب ورئيس وزراء المملكة المتحدة
        <br />
        إن الإجابة الوحيدة على الهزيمة هي الإنتصار
      </div>
    ),
    cover_pic: "https://www.ft.com/__origami/service/image/v2/images/raw/http%3A%2F%2Fcom.ft.imagepublish.upp-prod-eu.s3.amazonaws.com%2F4d3a6426-35fe-11ea-ac3c-f68c10993b04?dpr=1&fit=scale-down&source=next&width=700",
    birthday: "1874 "  + " 30" + " نوفمبر",
    location: "المملكة المتحدة",
    website: "https://winstonchurchill.org/",
    character_link: "Churchill_UK",
  },
  {
    character: "كارل ماركس",
    character_pic:
      "https://cdn.britannica.com/22/59822-050-98F24569/Karl-Marx-1870.jpg",
    username: "@realKarlMarx",
    bio: (
      <div>
        كاتب 📚
        <br />
        رأس المال 👈
        <br />
        البيان الشيوعي 👈
        <br />
        الإيديولوجية الألمانية 👈
        <br />
        بؤس الفلسفة 👈
        <br />
        ليس للعمال شيئ يخسرونه سوى أغلالهم, ولكن أمامهم عالم رحب ليكسبوه
      </div>
    ),
    cover_pic: "http://www.marxistreview.asia/wp-content/uploads/2017/05/karl-marx-friedrich-engels-en-la-imprenta-de-la-rheinische-zeitung-colonia-museo-marx-engels-moscc3ba-e29c86-e-chapiro-c2a9-c3b1c3a1ngara-marx1-810x505.jpg",
    birthday:  "1818 "  + " 5" + " ماي",
    location: "المملكة المتحدة",
    website: "https://en.wikipedia.org/wiki/Karl_Marx",
    character_link: "marx",
  },
  {
    character: "حنبعل",
    character_pic:
    "https://upload.wikimedia.org/wikipedia/commons/e/ef/Mommsen_p265.jpg",
    username: "@hannibalOfficial",
    bio: (
      <div>
       الحساب الرسمي للقائد القرطاجي حنبعل
      </div>
    ),
    cover_pic: "https://cdnb.artstation.com/p/assets/images/images/033/053/565/large/alex-nice-hannibal-sm4.jpg?1608242935",

    birthday: (
      <div style={{display: "flex", justifyContent: "space-between"}}>
        <div>قبل الميلاد</div>
        <div style={{visibility: "hidden"}}>..</div>
       <div>275</div>
      </div>
    ),
    location: "قرطاج",
    website: "https://en.wikipedia.org/wiki/Adolf_Hitler",
    character_link: "hannibalOfficial",
  },
  {
    character: "توماس جيفرسون",
    character_pic:
    "https://upload.wikimedia.org/wikipedia/commons/4/46/T_Jefferson_by_Charles_Willson_Peale_1791_2.jpg",
    username: "@JeffersonOfficial",
    bio: (
      <div>
        الرئيس الـ3 للولايات المتحدة الأمريكية
      </div>
    ),
    cover_pic: "https://in2english.net/wp-content/uploads/2018/10/New-York-City-800x531.jpg",
    birthday: "1743 "  + " 13" + " أفريل",
    location: "الولايات المتحدة الأمريكية",
    website: "https://www.whitehouse.gov/",
    character_link: "jeffersonOfficial",
  },
  {
    character: "فرانكلين دي روزفلت",
    character_pic:
      "https://ids.si.edu/ids/deliveryService?id=NPG-NPG_64_14Roosevelt-000001",
    username: "@fdr",
    bio: (
      <div>
        الرئيس الـ32 للولايات المتحدة الأمريكية
      </div>
    ),
    cover_pic: "https://newsela.imgix.net/article_media/2020/01/lib-presidential-election-1932-16dac958.jpg?auto=compress%2C%20enhance&fit=crop&crop=focalpoint&fp-x=0.5&fp-y=0.58&fp-z=1&ar=16%3A9&q=50&w=689",
    birthday: "1882 "  + " 30" + " جانفي",
    location: "الولايات المتحدة الأمريكية",
    website: "www.whitehouse.gov/",
    character_link: "fdr",
  },
  {
    character: "الأخبار 24/24",
    character_pic:
      "https://cdn.britannica.com/57/146057-050-697A470D/Adolf-Hitler.jpg",
    username: "@realAdolfHitler",
    bio: (
      <div>
        الأخبار على مدار الساعة
      </div>
    ),
    cover_pic: "https://learndojo.org/wp-content/uploads/2019/08/GCSE-History-Life-in-Nazi-Germany-1-768x518.jpg",
    birthday: "1889 "  + " 30" + " أفريل",
    location: "ألمانيا",
    website: "https://en.wikipedia.org/wiki/Adolf_Hitler",
    character_link: "news",
  },
];
